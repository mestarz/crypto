...
class AvgRSI:
    ...
    def _run_up(self, rsi_list: np.array):
        ...
        if rsi < self.rsi_down:
            ...
            if real_position < min_position:
                self._set_long_position(min_position)
            ...
            if real_position > max_position:
                self._set_long_position(max_position)
    ...
    def _run_down(self, rsi_list: np.array):
        ...
        if rsi > self.rsi_up:
            ...
            if real_position < min_position:
                self._set_short_position(min_position)
            ...
            if real_position > max_position:
                self._set_short_position(max_position)
    ...
    def _set_long_position(self, percentage: float):
        long, short = self.cfg.api.get_position_size_long_and_short(self.cfg.trade_config.coin)
        """清掉空头仓位"""
        if short > 0:
        ...
        self.exec.sleep(1)
        balance = self.cfg.api.get_account_balance(self.cfg.trade_config.ccy)
        count = int((balance * self.lever / self.current_price // self.ct_val + long) * percentage - long)
        self.cfg.logger.debug(f"设置多头仓位: {count}")
        if count < 0:
            r = self.cfg.api.market_long_sell(self.cfg.trade_config.coin, -count)
        else:
            r = self.cfg.api.market_long_buy(self.cfg.trade_config.coin, count)
        print(r)

    def _set_short_position(self, percentage: float):
        long, short = self.cfg.api.get_position_size_long_and_short(self.cfg.trade_config.coin)
        """清掉多头仓位"""
        if long > 0:
        ...
        self.exec.sleep(1)
        balance = self.cfg.api.get_account_balance(self.cfg.trade_config.ccy)
        count = int((balance * self.lever / self.current_price // self.ct_val + short) * percentage - short)
        self.cfg.logger.debug(f"设置空头仓位: {count}")
        if count < 0:
            r = self.cfg.api.market_short_sell(self.cfg.trade_config.coin, -count)
        else:
            r = self.cfg.api.market_short_buy(self.cfg.trade_config.coin, int(count))
        print(r)

    def _clear_long(self):
        """清掉多头仓位"""
        long, _ = self.cfg.api.get_position_size_long_and_short(self.cfg.trade_config.coin)
        if long > 0:
            self.cfg.logger.debug(f"清空多头仓位: {long}")
            self.cfg.api.market_long_sell(self.cfg.trade_config.coin, long)

    def _clear_short(self):
        """清掉空头仓位"""
        _, short = self.cfg.api.get_position_size_long_and_short(self.cfg.trade_config.coin)
        if short > 0:
            self.cfg.logger.debug(f"清空空头仓位: {short}")
            self.cfg.api.market_short_sell(self.cfg.trade_config.coin, short)
